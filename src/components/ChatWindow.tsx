    ...
    <div className={`flex-1 flex flex-col glass-card rounded-2xl overflow-hidden shadow-lg ${!mobileVisible ? "hidden md:flex" : "flex"}`}>
      ...
      <div className="p-4 md:p-5 border-b border-[rgba(27,54,93,0.06)] flex items-center justify-between bg-white/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-[#002046] text-sm">{conversation.user_name}</h3>
            <p className="text-[10px] text-[#44474e]/50">{conversation.subject}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="w-8 h-8 rounded-full flex items-center justify-center text-[#44474e]/40 hover:text-[#002046] hover:bg-[rgba(27,54,93,0.04)] transition-all" title="Call">
            <span className="material-symbols-outlined text-[18px]">call</span>
          </button>
          <button className="w-8 h-8 rounded-full flex items-center justify-center text-[#44474e]/40 hover:text-[#002046] hover:bg-[rgba(27,54,93,0.04)] transition-all" title="More options">
      ...
      <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-4 bg-[rgba(239,244,255,0.3)]">
        ...
        <div className="flex justify-center">
          <span className="px-3 py-1 rounded-full bg-[#e5eeff] text-[10px] font-bold text-[#44474e]/50 uppercase tracking-widest">
            Today
          </span>
        </div>

        {messages.map((msg) => {
          const isStaff = msg.sender_type !== "user";
          return (
            <div
              key={msg.id}
              className={`flex items-end gap-2.5 max-w-[85%] ${isStaff ? "ml-auto flex-row-reverse" : ""}`}
            >
              <div
